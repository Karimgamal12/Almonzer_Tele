...
def main_menu():
    text = f"**- [سـورس افـاتـار](https://t.me/V_P_N_8)\
        \n المسـاعد\
        \n\
        \n◽ V_P_N_8لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("jepiq"):
            buttons = [
                (
                    ...
                    Button.url(" V_P_N_8 UsᴇʀBoᴛ", "https://t.me/V_P_N_8"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./V_P_N_8", "troll.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس V_P_N_8🧸♥",
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./V_P_N_8", "secrets.txt")
            try:
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس V_P_N_8🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./V_P_N_8", "hide.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس V_P_N_8🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="© V_P_N_8 Help",
                description="Help menu for V_P_N_8",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت V_P_N_8🧸♥](https://t.me/V_P_N_8)**\
            \n\
            \n❤ V_P_N_8هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/V_P_N_81)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
