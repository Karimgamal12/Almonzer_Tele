...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await jepiq.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/38b8d6343df1d0aed46fa.jpg",
                caption="**᯽︙ بــوت الـمـنزريـعـمـل بـنـجـاح ✓ **\n**᯽︙ ارسل `.الاوامر` لرؤية اوامر السورس**",
                buttons=[(Button.url("سورس افـاتـار", "https://t.me/V_P_N_8"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
    ...
    try:
        if msg_details:
            ...
            text = (
                message.text
                + "\n\n**᯽︙اهلا وسهلا لقد قمت باعاده تشغيل بـوت الـمـنزرتمت بنجاح**"
            )
...
jepthon = {"@V_P_N_8", "@Help_MonZer_7_P", "@Source_Monzer"}
async def saves():
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        photobt = await jepiq.upload_file(file="JepIQ/razan/resources/start/Av.jpg")
        _, groupid = await create_supergroup(
            "مجموعة أشعارات الـمـنزر", jepiq, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        photobt = await jepiq.upload_file(file="JepIQ/razan/resources/start/Avb.jpg")
        _, groupid = await create_supergroup(
